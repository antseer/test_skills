...
|--------|------|------|------|--------|--------|
| `strategy_type` | enum | 是 | insurance / sports_quant / bayesian_arb / custom | — | `insurance` |
| `market_question` | string | 是 | Polymarket 市场问题描述 | — | `"Will BTC exceed $100K by end of Q2 2026?"` |
| `market_yes_price` | float | 是 | 当前 YES 市场价格（0-1之间） | — | `0.12` |
| `your_true_prob` | float | 是 | 你估算的真实概率（0-1之间） | — | `0.05` |
| `bankroll` | float | 是 | 总本金（美元） | — | `10000` |
...
| Polymarket 实时市场价格 | 无（用户手动输入） | — | 不覆盖 |
| 体育/气象/球员疲劳数据 | 无（需外部 API） | — | 不覆盖 |

**说明：**
- `ant_market_sentiment` 仅对 crypto 相关 Polymarket 市场有参考价值，政治/体育类不覆盖
- Polymarket 实时价格需用户手动输入或通过 Polymarket 官方 API（`https://clob.polymarket.com/`）获取
...
3. 用户提供了 market_yes_price 和 your_true_prob 两个核心参数
...
1. 读取用户输入的 `strategy_type` 和 `edge_description`
2. 检查是否符合以下死亡模式：
   - **动量交易特征**：追涨杀跌、基于价格趋势、"最近一直涨/跌"
   - **新闻/直觉驱动**：没有量化模型、"我觉得"、基于公开新闻判断
   - **跟单操作**：复制他人交易、"某大户也在买"、事后跟进
   - **纯价差套利**：仅依赖 LMSR 价差、无独立概率估算
3. `strategy_type = custom` 时，从 `edge_description` 的描述中判断最接近哪类
4. 若匹配死亡模式，输出 DANGER 警告并说明对应死亡率（继续后续步骤供参考，不强制中断）
5. 若匹配存活策略类型（insurance/sports_quant/bayesian_arb），输出 SAFE 或 WARNING

**输出**：策略健康状态（SAFE / WARNING / DANGER）及原因说明
