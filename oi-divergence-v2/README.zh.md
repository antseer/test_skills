...
2. **三条件交叉验证庄家控盘**：暴涨 + 合约量 Top 20 + 沉寂多日后突然大阳线
...
| 三条件交叉验证 | 24H 涨幅 50%+ × 合约量 Top 20 × 突然拉升形态 |
| 5 分钟 OI 背离 | 双层检测：30 分钟滚动窗口 + 逐根 K 线分析 |
| 极端风控 | 爆仓价控制在入场价 10x 以上，防庄家反拉 |
...
扫描时间: 2026-04-06 14:30 UTC
数据源: Binance（仅限）
筛选结果: 247 个币种 → 3 个 24H>50% → 2 个合约拉盘确认 → 1 个 K 线形态符合 → 1 个 OI 背离
...
⚠️ 发现 1 个庄家控盘做空信号：
1. **SIRENUSDT** (强背离) — 24H +85%, 合约量 Top 5, OI 持续下降中
...
- 合约：SIRENUSDT
- 入场区间: $0.0774 - $0.0782
- 止损位: $0.0081（最近高点上方 1.5%）
- 止盈位 1: $0.0052（24H 低点）
- 止盈位 2: $0.0068（背离起始价格）
- 盈亏比: 3.2:1
- 爆仓价: > $0.078（控制在入场价 10x+，防止庄家拉爆）
...
| 币种 | 24H 涨幅 | 淘汰阶段 | 原因 |
|------|----------|----------|------|
| ALPACAUSDT | +55% | 条件 2 | 合约量未进 Top 20 |
| MOVEUSDT | +70% | 条件 3 | 近 30D 波动率高，非突然拉升 |

### 风险提示

- 此分析基于 5min 级别数据，信号有效窗口短，需及时跟进
- 庄家可能二次拉升后再砸，入场需等确认信号
- 爆仓价设在入场价 10x+ 是极端风控，仓位占比极低
- 仅供学习参考，不构成投资建议
```
