...
description: "交易所巨鲸仓位反向信号分析。监控交易所（Bitfinex、Binance等）的仓位极端变化（OI、多空比、保证金多头），基于统计回测生成反向操作信号。当用户提到巨鲸仓位、whale position、Bitfinex margin longs、保证金多头信号、巨鲸反向指标、whale contrarian、大户持仓异动、OI极端变化、仓位背离、鲸鱼做多做空信号时触发。也适用于用户想了解某个币种是否出现仓位极端信号、是否应该反向操作的场景。"
metadata:
...
| 参数名 | 类型 | 必填 | 说明 | 默认值 |
|--------|------|------|------|--------|
| symbol | string | 是 | 目标代币符号 | - |
| exchange | string | 否 | 目标交易所 | binance |
| position_type | string | 否 | 仓位类型：futures_oi / long_short_ratio | futures_oi |
| lookback_days | int | 否 | 历史回看天数 | 180 |
| rolling_window | int | 否 | 滚动变化率窗口（天） | 30 |
| increase_threshold | float | 否 | 仓位大涨阈值（%） | 15.0 |
| decrease_threshold | float | 否 | 仓位大跌阈值（%，绝对值） | 10.0 |
| signal_eval_windows | list | 否 | 信号评估窗口（天） | [14, 30] |
...
- symbol: 用户指定的 symbol（如 BTC）
- exchange: 用户指定或默认 binance
...
对 Step 2 的仓位序列，逐日计算 rolling_window 天的百分比变化：
...
- `position_change_pct > increase_threshold` --> 标记 **WHALE_ACCUMULATION**（巨鲸抄底，反向信号：考虑做空）
- `position_change_pct < -decrease_threshold` --> 标记 **WHALE_DISTRIBUTION**（巨鲸止盈，反向信号：考虑做多）

记录每个信号的触发日期和变化幅度。

### Step 6: 信号回测评估

对 Step 5 的每个历史信号，计算触发后 signal_eval_windows 天内的价格变化：
